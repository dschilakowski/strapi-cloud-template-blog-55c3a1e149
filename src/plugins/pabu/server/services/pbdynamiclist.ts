  ...
  async updateDynamicList(id: number, data: any) {
    ...
    if (_locale === defaultLocale) {
      ...
      if (localizedDynamicList) {
        // localized dynamiclist does exist
        ...
        // TODO do not clone if the file was uploaded before at the start
        // of the controller
        // since we are uploading manually we only assign the ID to the attribute
        // normally the field would be filled with the strapi file object containing
        // id, mime, ext... so we can check if this is a full strapi file object
        // to recognize if we have to clone or not
        ...
        if (newLocalizedDynamicList) {
          ...
          console.log("sanitize and return newLocalizedDynamicList");
