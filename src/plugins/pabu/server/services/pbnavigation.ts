...
export default ({ strapi }: { strapi: Strapi }) => ({
  async cmsListNavigations(locale: string) {
    ...
    console.log(`cms searching for navigations with locale ${locale}`);
  ...
  async cmsUpdateNavigation(id: number, data: any) {
    console.log("navigation update! " + id);

    // this is update so get the locale from incoming request data
    let locale = data.locale;
    const strapiLocaleExists = await i18nLocaleExists(locale);
    const strapiDefaultLocale = await i18nDefaultLocale(
      process.env.STRAPI_PLUGIN_I18N_INIT_LOCALE_CODE
    ...
    if (locale === strapiDefaultLocale) {
      // if it is default locale (this code wasnt modified)
      // this works exactly as it was without localization
      ...
      // if it is not default locale
      // search if this locale already exist and update the existing one
      // if not search the default locale and create the new one from the request data

      // get the default locale object
      // the name is unique so you can also search by name and default locale
      // another option would be to send the default locale Id as a query param
      // then you can update the localized one via request.data and find the
      // default one by query param id (defaultlocale ID)
      // like in findById
      ...
      console.log("navigation update for locale: " + locale);
      if (localizedNavigation) {
        ...
        // localized navigation does not exist
        ...
        try {
          ...
          // sync locales
          // every time you create or delete a localized entry you have to
          // synchronize all localizations arrays
          // ATTENTION: This will trigger a lifecycle hook with only
          // data.localizations as parameter you have to handle this
          // if there are lifecycle hooks for the model
  ...
  async cmsDeleteNavigation(id: number, locale: string) {
    console.log("navigation delete! " + id);
  ...
  async createNavigationJson() {
    ...
    // TODO look for a better solution to this. You can use the afterUpdate params
    // and set the new values manually...
  ...
  async checkNavigations(pageId: number, changedPage: any) {
    ...
    if (allNavigations) {
      for (let i = 0; i < allNavigations.length; i++) {
          ...
          if (page.subPages && page.subPages.length > 0) {
            ...
            for (let k = 0; k < page.subPages.length; k++) {
              ...
              if (subPage.pageId === changedPageId && !changedPage) {
                ...
                // Skipping 3rd-Level Checks (irrelevant)
                continue;
              }
        ...
        if (pageIsAffected) {
          console.log("[checknavigations] updateAndReturnPopulated");
  ...
  async cmsFindMainNavigation(locale: string) {
    ...
    if (locale === strapiDefaultLocale) {
      ...
      console.log(`cms searching for main navigation with locale ${locale}`);
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    if (!page.subPages || page.subPages.length === 0) {
      ...
      for (let j = 0; j < page.subPages.length; j++) {
        ...
        if (!pagesSubPage.pageId || !pagesSubPage.page) {
          // Note: Habe hier (^) einen eventuellen Typo (page -> pagesSubPage) gefixt. Ausgiebig testen.
          if (
            pagesSubPage.pageId === EXTERNAL_PAGE_ID &&
