...
const insertOrReplaceStrapiUpload = async (
  ...
  if (fileBuffer) {
    ...
    if (
      ...
      console.log("update existing media library file");
      try {
...
const createNewChunkUploadFile = async (
  ...
  if (uploadParams.fileSize === 0) {
    console.log("fileSize is 0 Bytes aborting upload...");
    return {
      status: "failed",
  ...
  if (
    ...
    console.log("filename contains no or more dots so this file is invalid");
    return {
      status: "failed",
  ...
  // TODO you could check the mimeType here already
 ...
 * console.log(generatedFilename); // e.g., "my_file_name_1a4f3b2c9e"
 */
 ...
 * console.log(suffix); // e.g., "1a4f3b2c9e"
 */
