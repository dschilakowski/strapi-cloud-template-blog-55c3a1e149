...
export default {
  ...
  async createOrUpdateFile(
    foldername: string, // assets | uploads // at the moment no other folders are available
    filename: string,
    ...
    console.log(`Trying to update ${path}${filename}.${fileExtension}`);
    try {
      await access(path);
      if (isBase64) {
      ...
      console.log(`updated ${filename}.${fileExtension}`);
      return true;
    } catch (error) {
      try {
        console.log(
        ...
        console.log(`created ${filename}.${fileExtension}`);
        return true;
      } catch (error) {
        console.log(error);
        console.log(
  ...
  appendToFile: async (
    ...
    const path = isPrivate
      ? `${process.cwd()}/private/${foldername !== "" ? foldername + "/" : ""}`
      : `${process.cwd()}/public/${foldername !== "" ? foldername + "/" : ""}`;
    // console.log(`Trying to update ${path}${filenameWithExtension}`);
    try {
      if (fs.existsSync(`${path}${filenameWithExtension}`)) {
        ...
        // console.log(`appended to ${filenameWithExtension}`);
        return true;
      } else {
        console.log(
  ...
  renameFile: (
    ...
    const path = isPrivate
      ? `${process.cwd()}/private/${foldername !== "" ? foldername + "/" : ""}`
      : `${process.cwd()}/public/${foldername !== "" ? foldername + "/" : ""}`;
    // console.log(`Trying to rename ${path}${oldFilename} to ${newFilename}`);
    try {
  ...
  getFileStats: (
    ...
    try {
      ...
      console.log(error);
      console.log(`could not get file stats ${path}${filename}  - aborted`);
      return null;
    }
  },
  ...
  removeFile: (
    ...
    const path = isPrivate
      ? `${process.cwd()}/private/${foldername !== "" ? foldername + "/" : ""}`
      : `${process.cwd()}/public/${foldername !== "" ? foldername + "/" : ""}`;
    console.log(`Trying to remove ${path}${filename}`);
    ...
    try {
      ...
      console.log(error);
      console.log(`could not delete file ${path}${filename}  - aborted`);
      return false;
    }
  },
  ...
  checkMimeTypeFromFile: async (
    ...
    if (
      fileType &&
      // uncheckedIncomingMimeType !== "" &&
      uncheckedIncomingMimeType !== fileType.mime
    ) {
      console.log(
    ...
    if (
      ...
      console.log(`[checkMimeTypeFromFile]: ${fileType.mime} is not allowed`);
    }
  ...
  getFileStream: async (
    ...
    try {
      ...
      console.log(`could not create readstream for file ${path}${filename}`);
    }
    return null;
  },
  ...
  getFileBuffer: (
    ...
    try {
      if (fs.existsSync(`${path}${filename}`)) {
        ...
        console.log(
          `could not create buffer for file ${path}${filename} file does not exist`
        );
      }
    } catch (error) {
      console.log(`could not create buffer for file ${path}${filename}`);
    }
    return null;
  },
  /**
