...
import { countMediaInObjectById, getUploadService } from "../utils/functions";
import {
  i18nDefaultLocale,
...
export default ({ strapi }: { strapi: Strapi }) => ({
  ...
  async findReferences(fileId: string) {
    ...
    const searchList = [PAGE_MODULE_UID];
  ...
  async deleteFile(id: number) {
    ...
    return await getUploadService("upload").remove(file);
  },
  async updateFile(updateFileRequest: UpdateFileRequest) {
    ...
    if (strapiFile.name !== updateFileRequest.file.name) {
      ...
      try {
        ...
        console.log(error);
        console.log("could not update strapi file");
        return;
      }
