...
export default {
    ...
    if (data) {
      ...
      for (const [key, value] of Object.entries(data)) {
        ...
        if (value && value.id) {
          ...
          console.log(`flatData: ${key}`);
          console.log(settingComponent.flatData);
  ...
  async regenerateSettingsJSONs(settings: Array<String> = ["glbl", "cesstr"]) {
    for (const setting of settings) {
      console.log(`regenerateSettingsJSONs: ${setting}-JSON.`);
      ...
      if (settingData && setting === "glbl") {
        console.log(`regenerateSettingsJSONs: Regenerate global.css`);
