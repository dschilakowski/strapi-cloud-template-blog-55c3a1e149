...
export const synchronizeLocalizations = async (
  ...
  // Current existing localizations
  let existingEntityLocalizations = defaultLocaleEntry.localizations;

  // console.log("- existingEntityLocalizations -");
  // console.log(existingEntityLocalizations);
  ...
  if (existingEntityLocalizations.length === updatedEntityLocalizations) {
    result = true;
    console.log(`Success: Updated all ${api} localizations:`);
  }

  if (!result) {
    console.log(`Error: Couldn't update all ${api} localizations:`);
  }
