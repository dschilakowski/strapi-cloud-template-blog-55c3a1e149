...
export default ({ strapi }: { strapi: Strapi }) => ({
  ...
  async suggest(ctx: Context) {
    ...
    if (
      ...
      // || !(await i18nLocaleExists(_locale))
    ) {
      return [];
      // TBD: Do we want to return results in defaultLocale?
      // _locale = await i18nDefaultLocale(
    ...
    for (const page of pages) {
      ...
      for (const pageContent of page.content) {
        // a)
        // We might want to stick with only headline-CE-Elements for search-suggestions.
        // Search-Suggest by default searches only in page.name & headline-contentelement.
        // if (pageContent.__component === "pb.chdln") {
        //   if (
        // caseInsensitiveIncludes(pageContent.primaryHeadline, lowerCaseSearchString);
        //   ) {
        //     results.add(pageContent.primaryHeadline);
        //   }
        //   if (
        //   caseInsensitiveIncludes(pageContent.secondaryHeadline, lowerCaseSearchString)
        //   ) {
        //     results.add(pageContent.secondaryHeadline);
        //   }
        // }
